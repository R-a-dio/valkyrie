...
SELECT
	...
	COALESCE(ow.updated_at, TIMESTAMP('0000-00-00 00:00:00')) AS 'owner.updated_at',
	COALESCE(ow.deleted_at, TIMESTAMP('0000-00-00 00:00:00')) AS 'owner.deleted_at',
	COALESCE(ow.created_at, TIMESTAMP('0000-00-00 00:00:00')) AS 'owner.created_at',
...
func (ss ScheduleStorage) Latest() ([]*radio.ScheduleEntry, error) {
	...
	err := sqlx.Select(handle, &tmp, latestScheduleQuery)
	if err != nil {
...
func (ss ScheduleStorage) History(day radio.ScheduleDay, limit, offset int64) ([]radio.ScheduleEntry, error) {
	...
	err := sqlx.Select(handle, &history, query, day, limit, offset)
	if err != nil {
		return nil, errors.E(op, err)
	}
	return history, nil
